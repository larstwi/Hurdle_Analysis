...
@st.cache_data
def load_data():
    data = pd.read_excel('data/Analyse22.xlsx')
    return data
...
def show_row_differences(df, selected_row):
    ...
    result = pd.concat([df[["Jahr"]], df[["Name"]], df[["Wettkampf"]], differences], axis=1)
...
def main():
    ...
    athletes = st.sidebar.multiselect("Athlet:in wählen", options=data["Name"].unique(), default=[])
